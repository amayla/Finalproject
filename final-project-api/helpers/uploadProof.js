...
let multerStorageConfig =  multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        if(file.mimetype.split('/')[1] == 'jpeg'){
            cb(null, `${moment(Date.now()).format("DDMMYYYYHHmmss")}-${crypto.randomBytes(8).toString("hex")}.jpg`)
        } else {
            cb(null, `${moment(Date.now()).format("DDMMYYYYHHmmss")}-${crypto.randomBytes(8).toString("hex")}.${file.mimetype.split('/')[1]}`)
        }
    }
})
